...
const RoomDetail = () => {
  ...
  const handleReserve = async () => {
    ...
    if (!checkIn || !checkOut) { toast.error("Selecione as datas de check-in e check-out"); return; }
    ...
    if (error) {
      if (error.code === "23505") {
        toast.error("Já existe uma reserva para este quarto nestas datas!");
      } else {
    ...
    <div className="container-page py-6">
      ...
      <div className="mb-6 grid grid-cols-1 gap-2 lg:grid-cols-3 lg:grid-rows-2">
        <div className="lg:col-span-2 lg:row-span-2">
          <img src={room.images[activeImage]} alt={room.title} className="h-64 w-full rounded-xl object-cover sm:h-80 lg:h-full" />
        </div>
        {room.images.slice(1, 3).map((img, i) => (
          <button key={i} onClick={() => setActiveImage(i + 1)} className="hidden overflow-hidden rounded-xl lg:block">
            <img src={img} alt={`${room.title} foto ${i + 2}`} className="h-full w-full object-cover transition-transform hover:scale-105" />
          </button>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
        <div className="lg:col-span-2">
          ...
          <h1 className="mb-2 font-heading text-2xl font-bold text-foreground sm:text-3xl">{room.title}</h1>
          <div className="mb-4 flex items-center gap-4 text-sm">
            ...
            <span className="flex items-center gap-1 text-muted-foreground"><Users className="h-4 w-4" /> até {room.guests} hóspedes</span>
          </div>
          ...
          <Separator className="mb-6" />
          <h2 className="mb-4 font-heading text-lg font-semibold text-foreground">Avaliações ({allReviews.length})</h2>
          ...
          <div className="mb-6 rounded-xl border border-border bg-card p-4">
            ...
            <textarea value={comment} onChange={(e) => setComment(e.target.value)} placeholder="Conte como foi sua experiência..." className="mb-3 w-full rounded-lg border border-input bg-background p-3 text-sm outline-none focus:ring-2 focus:ring-ring" rows={3} />
            <Button onClick={handleComment} disabled={!comment.trim()}>Enviar avaliação</Button>
          </div>
        ...
        <div>
          <div className="sticky top-20 rounded-xl border border-border bg-card p-5 shadow-elevated">
            <div className="mb-4 flex items-baseline gap-2">
              ...
              {room.originalPrice && <span className="text-sm text-muted-foreground line-through">R$ {room.originalPrice}</span>}
            ...
            <div className="mb-4 space-y-3">
              ...
              <div>
                ...
                <select value={guests} onChange={(e) => setGuests(Number(e.target.value))} className="w-full rounded-lg border border-input bg-background p-2.5 text-sm outline-none focus:ring-2 focus:ring-ring">
            ...
            {checkIn && checkOut && (
              <div className="mb-4 space-y-2 text-sm">
                <div className="flex justify-between"><span className="text-muted-foreground">R$ {room.price} x {nights} noite(s)</span><span className="text-foreground">R$ {subtotal}</span></div>
                <div className="flex justify-between"><span className="text-muted-foreground">Taxa da plataforma</span><span className="text-foreground">R$ {fee}</span></div>
                <Separator />
                <div className="flex justify-between font-semibold"><span className="text-foreground">Total</span><span className="text-foreground">R$ {total}</span></div>
              </div>
            )}
